...
import { BANCO, ESCALA, TOTAL_PREGUNTAS } from '../src/data/banco.ts';
...
L.push('---');
L.push('');
L.push(`*Total: ${TOTAL_PREGUNTAS} preguntas · 45–60 min en reunión de levantamiento por área (15–20 min si se responde individualmente). Instrumento orientativo de autodiagnóstico; no constituye asesoría legal.*`);
L.push('');

const out = path.join(path.dirname(fileURLToPath(import.meta.url)), '..', 'docs', 'cuestionario.md');
writeFileSync(out, L.join('\n'));
console.log(`OK: ${out} (${TOTAL_PREGUNTAS} preguntas)`);
